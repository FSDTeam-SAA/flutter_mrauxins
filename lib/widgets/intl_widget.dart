...
class AppIntlPhoneField extends StatefulWidget {
  ...
  @override
  _AppIntlPhoneFieldState createState() => _AppIntlPhoneFieldState();
}
...
class _AppIntlPhoneFieldState extends State<AppIntlPhoneField> {
  ...
  Future<void> handlePasteNumber(String value) async {
    if (widget.initialCountryCode == null && number.startsWith('+')) {
      number = number.substring(1);
      // parse initial value
      _selectedCountry = countries.firstWhere(
...
class _CountryPickerDialogState extends State<CountryPickerDialog> {
  ...
  void initState() {
    ...
    _filteredCountries = sortCountries(widget.filteredCountries);

    super.initState();
  }

  List<Country> sortCountries(List<Country> filteredCountries) {
    // Prioritize UK and US at the top
    const priorityCountries = ['United Kingdom', 'United States'];
    final mutableCountries = List<Country>.from(filteredCountries);
    mutableCountries.sort((a, b) {
      if (priorityCountries.contains(a.name) &&
          priorityCountries.contains(b.name)) {
        return a.name.compareTo(b.name); // Sort UK/US among themselves
      } else if (priorityCountries.contains(a.name)) {
        return -1; // Keep UK/US at the top
      } else if (priorityCountries.contains(b.name)) {
        return 1; // Keep UK/US at the top
      }
      return a.name.compareTo(b.name); // Alphabetical sort for others
    });
          ...
          children: <Widget>[
            Padding(
              ...
              child: TextField(
                ...
                onChanged: (value) {
                  _filteredCountries =
                      sortCountries(widget.countryList.stringSearch(value));
                  if (mounted) setState(() {});
                },
              ),
            ),
